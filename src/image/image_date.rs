...
impl Image {
    fn parse_exif_datestring(
        ...
        let date = Utc.with_ymd_and_hms(
            image_dates[0].parse::<i32>()?,
            image_dates[1].parse::<u32>()?,
            image_dates[2].parse::<u32>()?,
            image_dates[3].parse::<u32>()?,
            image_dates[4].parse::<u32>()?,
            image_dates[5].parse::<u32>()?,
        );
        let date = match date {
    ...
    fn get_date_exif(path: &PathBuf) -> Result<DateTime<Utc>, ImageParseError> {
        ...
        let date = Self::parse_exif_datestring(field).map_err(|_| ImageParseError::Exif)?;
    ...
    fn get_date_whatsapp(file_name: String) -> Result<DateTime<Utc>, ImageParseError> {
        ...
        let naive_date = NaiveDate::parse_from_str(date_str, "%Y%m%d")
            .map_err(|_| ImageParseError::InvalidName)?;
    ...
    pub fn get_image_date(path: &PathBuf) -> Result<DateTime<Utc>, ImageParseError> {
        ...
        if file_name.starts_with("IMG") && file_name.contains("WA") {
            date = Self::get_date_whatsapp(file_name)?;
            return Ok(date);
        }

        date = match Self::get_date_exif(path) {
            Ok(v) => v,
            Err(_) => Self::get_date_imagesorter(file_name)?,
        };
