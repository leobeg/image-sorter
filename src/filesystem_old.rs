...
pub fn create_file_list(
    path: &PathBuf,
) -> Result<Vec<(PathBuf, DateTime<Utc>)>, Box<dyn std::error::Error>> {
...
pub fn move_images_to_sort(
    source_path: &PathBuf,
) -> Result<Vec<(u32, PathBuf, DateTime<Utc>)>, SortError> {
    ...
    for (path, date) in file_list {
        ...
        let copy = copy_image(
            &path,
            &target_path,
            format!("{i}_{year}-{month}-{day}.{extension}"),
        )
        .map_err(|_| SortError::Copy)?;
